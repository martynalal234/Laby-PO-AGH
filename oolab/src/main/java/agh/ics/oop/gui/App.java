...
public class App extends Application {
    ...
    private void drawMap(Stage primaryStage){
        ...
        for (int i = 0; i <= rangeY; i++) {
            ...
            //tworzenie label dla pierwszej wspolrzednej z lewej w poszczegolnych wierszach
            ...
            for (int j = 0; j < rangeX+1; j++) {
                if (i == 0) {
                    //tworzenie label dla pierwszej wspolrzednej z gory w poszczegolnych kolumnach
        ...
        //tworzenie dodatkowego label w lewym gornym rogu
        label = new Label("x/y");
        grid.getColumnConstraints().add(new ColumnConstraints(width));
        grid.getRowConstraints().add(new RowConstraints(height));
        grid.add(label, 0, 0);
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            MoveDirection[] directions2 = new OptionsPharser().parse(args);
